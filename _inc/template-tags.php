...
if ( ! function_exists( 'yom_posted_date' ) ) {
	function yom_posted_date() {
		$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
...
if ( ! function_exists( 'yom_tags_list' ) ) {
	function yom_tags_list() {
...
if ( ! function_exists( 'yom_cats_list' ) ) {
	function yom_cats_list() {
		/* translators: used between list items, there is a space after the comma */
		...
		if ( $the_cats ) {
			foreach ( $the_cats as $cat ) {
				if ( 'Uncategorized' != $cat->name ) {
					// leave 'Uncategorized' out of list
